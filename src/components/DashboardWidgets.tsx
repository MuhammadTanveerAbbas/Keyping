...
function StatCard({
 ...
 return (
  <div
   className={cn(
     "rounded-xl border border-slate-200/80 bg-white p-4 space-y-2.5 hover:shadow-card-hover hover:-translate-y-0.5 transition-all duration-300",
    className,
   )}
  >
...
function ProviderUptime({ tests }: { tests: KeyTest[] }) {
 ...
 return (
   <Panel title="Provider uptime" description="Valid tests / total per provider">
    <div className="space-y-3 bg-[linear-gradient(rgba(0,0,0,0.02)_1px,transparent_1px),linear-gradient(90deg,rgba(0,0,0,0.02)_1px,transparent_1px)] bg-[length:16px_16px] rounded-lg p-3">
    {providers.slice(0, 6).map((p) => (
     <div key={p.id} className="space-y-1.5">
      ...
      {p.daysSince !== null && p.daysSince > 7 && (
       <p className="font-mono text-[10px] text-amber-500 flex items-center gap-1">
