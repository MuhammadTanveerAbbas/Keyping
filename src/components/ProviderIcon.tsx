...
export function ProviderIconBadge({ provider, className }: { provider: string; className?: string }) {
 return (
  <div className={cn("h-8 w-8 rounded-lg bg-blue-50 border border-blue-100 flex items-center justify-center", className)}>
   <ProviderIcon provider={provider} size="md" className="text-blue-600" />
  </div>
 );
}
