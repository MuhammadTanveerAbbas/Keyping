  ...
  <DashboardLayout>
   <PageShell width="md">
    ...
    {loading ? (
     ...
     <div className="space-y-3">
      {teams.map((team) => (
        <Panel key={team.id} noPadding className="transition-shadow duration-200 hover:shadow-sm">
         ...
         <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
           ...
           <div className="mt-0.5 flex flex-wrap items-center gap-2">
            {isOwner(team) && (
             <span className="inline-flex items-center gap-1 rounded border border-slate-200 bg-slate-50 px-1.5 py-0.5 font-mono text-[10px] text-slate-600">
         ...
         <div className="flex flex-wrap items-center gap-2 sm:shrink-0">
          <Button
           ...
           onClick={() => copyInviteLink(team.id)}
            className={cn("gap-1.5 h-9 transition-all duration-200", dashGhostBtn)}
           >
           ...
           <Button
            ...
            className={cn("h-9 transition-all duration-200", dashGhostBtn)}
          >
           ...
           <AlertDialog>
            <AlertDialogTrigger asChild>
             <Button
              variant="ghost"
              size="icon"
               className="h-9 w-9 text-slate-400 hover:bg-red-50 hover:text-red-600 transition-all duration-200"
             >
           ...
           <div className="space-y-2">
            {(members[team.id] || []).map((m) => (
             <div key={m.id} className="flex items-center gap-2 text-sm">
              <div className="flex h-7 w-7 items-center justify-center rounded-full bg-slate-100 font-mono text-xs text-slate-600">
