   ...
   <PageShell width="md">
    ...
    <div className="space-y-3">
     {rows.map((row, i) => (
      <div
       key={row.id}
        className={`rounded-xl border bg-white p-4 transition-shadow duration-200 ${
      ...
      >
       <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
         <span className="w-6 h-6 shrink-0 text-xs tabular-nums text-slate-400 bg-slate-100 rounded-full flex items-center justify-center">{i + 1}</span>
         ...
         <div className="flex items-center gap-2">
          ...
          {row.result.healthScore !== undefined && (
           <span className={`font-mono text-xs font-bold ${
            row.result.healthScore >= 80 ? "text-green-600" : row.result.healthScore >= 50 ? "text-amber-500" : "text-red-500"
           }`}>
         ...
         <AlertDialogContent className="bg-white border border-slate-200">
          ...
          <AlertDialogFooter>
           ...
           <AlertDialogAction onClick={() => removeRow(row.id)}
            className="bg-red-600 hover:bg-red-700 text-white font-sans text-sm rounded-xl">
            Remove
           </AlertDialogAction>
          </AlertDialogFooter>
         </AlertDialogContent>
    ...
    <div className="flex flex-wrap gap-2">
     <Button variant="outline" onClick={addRow} disabled={rows.length >= 10} className={dashGhostBtn + " border-slate-200"}>
     ...
     {hasResults && (
      <Button variant="outline" onClick={exportPdf} className={dashGhostBtn + " border-slate-200"}>
