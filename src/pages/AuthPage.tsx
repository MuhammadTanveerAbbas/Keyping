...
function AuthTerminal() {
 ...
 useEffect(() => {
  ...
  return (
   <div className="bg-[#050505] border border-blue-500/20 rounded-xl font-mono text-sm w-full max-w-sm overflow-hidden">
    <div className="bg-gradient-to-r from-[#050505] to-[#0a0a0a] border-b border-blue-500/20 px-4 py-2 flex items-center gap-2">
     <span className="w-2.5 h-2.5 rounded-full bg-red-500/80" />
     <span className="w-2.5 h-2.5 rounded-full bg-amber-500/80" />
     <span className="w-2.5 h-2.5 rounded-full bg-green-500/80" />
     <span className="font-mono text-xs text-blue-400/80 ml-2">keyping-terminal</span>
    </div>
    <div className="p-5">
     ...
     <span className="inline-block w-2 h-5 bg-blue-400 terminal-cursor ml-0.5 translate-y-0.5 mt-1" />
    </div>
   </div>
  );
}
...
export default function AuthPage() {
 ...
 return (
  <div className="min-h-screen flex">
   ...
   <div className="hidden lg:flex lg:w-[55%] bg-[#000000] flex-col relative overflow-hidden">
    ...
    <div className="absolute inset-0 pointer-events-none overflow-hidden">
     {[...Array(8)].map((_, i) => (
      <div
       key={i}
       className="particle absolute w-1 h-1 rounded-full bg-blue-500/40"
       style={
        {
         left: `${10 + i * 12}%`,
         top: `${15 + (i % 3) * 25}%`,
    ...
    <div className="relative z-10 p-8">
     <div className="flex items-center gap-2.5">
      <KeyPingLogo size={28} />
      <span className="font-display text-base font-bold text-blue-400">
       KeyPing
      </span>
     </div>
    </div>
    ...
    <div className="flex-1 flex flex-col items-center justify-center relative z-10 px-12">
     <AuthTerminal />
     <p className="font-mono text-sm text-blue-400/60 mt-8 text-center">
      Validate faster. Ship with confidence.
     </p>
    </div>
   ...
   {/* Right panel */}
   <div className="flex-1 flex items-center justify-center bg-white border-l border-blue-500/20 px-4 sm:px-6 py-12 min-h-screen">
    <motion.div
    ...
    >
     ...
     <div className="mb-8">
      ...
      <p className="font-sans text-sm text-slate-500 mt-1">
       {isSignUp
        ? "Start validating API keys in 30 seconds"
        : "Sign in to your KeyPing account"}
      </p>
     </div>
     ...
     {/* Email form - disabled until implemented */}
     <div className="space-y-3 opacity-50 pointer-events-none">
      ...
      {!isSignUp && (
       <div className="text-right">
        <button className="font-sans text-xs text-blue-600 hover:underline">
         Forgot password?
        </button>
       </div>
      )}
     ...
     <button
      disabled
      className="w-full mt-4 py-3 rounded-xl font-sans font-semibold text-sm transition-all opacity-50 cursor-not-allowed
     ...
     <p className="font-sans text-sm text-slate-500 text-center mt-5">
     ...
     <p className="font-mono text-[10px] text-slate-400 text-center mt-6">
      <Lock className="h-3 w-3 inline-block mr-1" /> Your keys never leave our edge functions
     </p>
    </motion.div>
   </div>
